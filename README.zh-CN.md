...
> **⚠️ Bilibili 限制说明**
> B 站视频通过嵌入式 iframe 播放，受浏览器跨域策略限制：
> - 遥控器的**暂停 / 播放**按钮对 B 站视频**无效**，只能在视频画面上直接操作
> - 视频播完后约 **3 秒**自动切到下一首（通过 B 站 API 获取时长推算，非精确检测）
> - 如果 B 站 API 无法返回时长（网络问题或 av 号视频），将**不会**自动切歌，需手动点击"跳过"
